...
data.ts <- ts(data$total.foreign.sim, frequency = 365)
...
```{r}
require(fpp)
detrended.lin <- data.ts - ma(data.ts, order = 3, centre = T)
# detrended.lin <- data.ts - predict(fit4)
plot(detrended.lin)
```

# Derivative of the data to reach the stationarity
...
```{r}
#decomposed <- decompose(data.ts,type = "multiplicative")
```


```{r}
#plot(na.omit(decomposed))
```
...
```{r}
#decomposed <- decompose(data.ts.diff,type = "additive")
#plot(decomposed)
...
```{r}
#require(fpp)
#fit <- Arima(data.ts, order=c(3,1,3))
#summary(fit)
```
...
```{r}
fit <- auto.arima(data.ts,stepwise=TRUE,trace=TRUE,ic = "bic")
```
...
data.ts %>% forecast(h=12) %>% autoplot() + autolayer(testing)
```
...
```{r}
#require(forecast)
#msts_cons <- msts(as.data.frame(diff(data.ts)), seasonal.periods = c(7,30,365))
